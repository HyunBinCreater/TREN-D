...
$(function(){
	$("#search_btn").click(function(){
		...
		$("#searchfrm").attr("action","search.do");
		$("#searchfrm").submit();
...
function page_load(pageNumber) {
    // 현재 스크롤 위치를 저장합니다.
    var scrollTop = window.scrollY;
    
    // AJAX를 통해 해당 페이지의 내용을 가져와서 content div에 업데이트
    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'search.do?pageIndex=' + pageNumber , true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            var ContentDiv = document.getElementById('newpro_content');
            // 기존 내용을 제거합니다.
...
<li></li>
<li></li>
<li><a href="search.do"><font color="#000"><b>새상품</b></font></a></li>
<li><a href="usedsearch.do">중고상품</a></li>
</ul>
...
<table class="list_tbl" >
	<tr><td class="font_style">새상품</td></tr>
	<c:if test="${newtotal<1}">
    	<tr><td>검색 결과가 없습니다.</td></tr>
     </c:if>
     <c:if test="${newtotal>0}">
	<c:forEach var="list" items="${list }">
	<tr class="hover_color"> 
		<td style="width:25%"> <a href="#"><img width="200"height="230" src="product_upload/${list.profilename }"></a></td>
	    <td style="vertical-align: middle; font-size:15px;text-align:left;">
							<a href="#">${list.proname  }</a>
							<p align="left">${list.proinfo  }</p>
						</td>
	    <td style="width:20%;vertical-align: middle; font-size:13px;">가격 ${list.proprice}원 </td> 
    </tr>
    </c:forEach>
   </c:if>
</table>
...
<div class="pagging" id="pagging">
	    ...
	    <a href="#" onclick="page_load(1)">[first]</a> 
	    </c:if>
	    <c:if test="${before > 0}">
	        <a href="#" onclick="page_load('${before}')">[before]</a> 
	    </c:if>
	    
	    <c:forEach var="p" begin="${svo.pageStartNo}" end="${svo.pageLastNo}">
	        <c:if test="${p != svo.pageIndex}">
	            <a href="#" onclick="page_load('${p}')">${p}</a>
	        </c:if>
	        <c:if test="${p == svo.pageIndex}">
	    ...
	    <c:if test="${next < svo.totalPageCnt}">
	        <a href="#" onclick="page_load('${next}')">[next]</a>
	    </c:if>
	    <c:if test="${svo.pageIndex != svo.totalPageCnt }">
	    <a href="#" onclick="page_load('${svo.totalPageCnt}')">[last]</a>
	    </c:if>
	</div>
</c:if>
</div>
